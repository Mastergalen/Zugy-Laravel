<?php

return [
    'about-us.title' => 'Cosa siamo',
    'about-us.desc' => '<p>Noi di Zugy operiamo proprio come un normale negozio di liquori al dettaglio ma con una piccola significativa differenza: visita il nostro sitoweb invece di andare in un negozio.Qui troverai un\'ampia selezione di bevande; potrai scegliere tra circa 50 tipologie di liquori.Basta scegliere quello che volete.Fate il vostro ordine e noi ve lo consegneremo alla vostra porta.Alla consegna puoi pagare con carta di credito, paypal o in contanti.</p>
<p>La tecnologia ha trasformato molte attività ma non ancora quella degli alcolici che non ha subito trasformazioni negli ultimi cento anni. Zugy sta rivoluzionando il modo con cui le persone degustano le bevande attraverso l\'uso di una moderna tecnologia che caratterizza il nostro modo di operare.</p>',
    'about-us.find-out-more' => 'Per saperne di più sui social media',
    'about-us.partners' => 'Soci',
    'about-us.partners-desc' => '<p>Zugy è orgogliosa di essere partner di <a href="http://securitycourier.it/">Security Courier Express</a> che ti porterà a casa  il tuo ordine.</p>
<p>Usiamo solo corrieri in bicicletta, garantendo la consegna entro 60 minuti senza emissione di CO2 <i class="fa fa-leaf"></i>.</p>',
    'contact.title' => 'Contatto',
    'contact.address' => 'Zugy Srl. <br>Alzaia Navigli Grande 36<br> Milano, 20141 <br> Italia',
    'contact.customer-service' => 'Sotto c\'e\' il servizio cliente - è possibile indirizzare tutte le domande, commenti o dubbi a questo numero.',
    'contact.email.general' => 'Per richieste generali',
    'contact.email.investors' => 'Per gli investitori:',
    'shop.title' => 'Negozio',
    'home.title' => 'Home',
    'home.meta_description' => 'Zugy è un negozio di liquori in linea a Milano che offre consegna nel giro di un\'ora.',
    'home.meta-title' => 'Consegna di alcol su-richiesta per Milano',
    'home.tagline' => 'La tua birra, vino o liquore favoriti, consegnati direttamente a casa tua.',
    'home.marketing.address.title' => 'Impostare l\'indirizzo',
    'home.marketing.address.desc' => 'Seleziona da una vasta gamma di bevande alcoliche che possono essere consegnate a casa tua',
    'home.marketing.time.title' => 'Ordine in pochi minuti',
    'home.marketing.time.desc' => 'Aggiungi ció che desideri al tuo carrello paga alla cassa ed é fatto.',
    'home.marketing.delivery.title' => 'Consegna a domicilio',
    'home.marketing.delivery.desc' => 'Iniziaremo subito a preparare il vostro ordine e lo consegneremo a casa vostra. Devi solo presentare un documento d\'identitá ai nostri simpatici fattorini e divertiti!',
    'home.exclusive' => 'Attualmente serviamo solo Milano',
    'home.milan' => 'Milano',
    'home.expansion' => 'Molti progietti da sviluppare',
    'privacy-policy' => 'Informativa sulla privacy',
    'team' => 'Team',
    'terms-and-conditions' => 'Termini e condizioni',
];
